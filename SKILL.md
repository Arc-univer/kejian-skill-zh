...
- 每个总结要点默认应附上源课件的页码或简短页码范围。仅在明确报告提取失败时才可省略引用。
...
- 保存一个名为 `Summary - <stem>.md` 的 Markdown 文件
- （可选）如果用户请求 PDF 输出，生成名为 `Summary - <stem>.tex` 的 LaTeX 源文件
- （可选）如果用户请求 PDF 输出且 LaTeX 环境可用，生成名为 `Summary - <stem>.pdf` 的编译后 PDF 文件
- 所有生成的文件归入一个名为 `Summary - <stem>` 的输出文件夹中，放在与源文件相同的目录内，除非用户明确请求其他位置
- 最终回复报告产物路径和遇到的任何阻碍
...
- 默认输出文件夹为当前工作目录下的 `Summary - <chosen-stem>`，除非用户指定了其他位置
...
- 如果在此步骤中将提取文本、OCR 输出或保存的笔记写入磁盘，必须写入输出文件夹内，绝不能放在源文件旁边。
- 阅读足够多的内容以理解文档的宏观结构。
...
- 默认为每个总结要点附上源页码引用。
- 优先使用精确页码。
- 仅当一个概念确实跨越几张相邻幻灯片时才使用简短页码范围。
- 不要为了省力而降级为非常宽泛的章节级范围。
- 如果因为提取模糊导致页码映射不确定，明确说明并在最终定稿前解决。仅在明确报告提取失败时才可接受缺失引用。
...
- 输出文件夹：`Summary - <stem>`，放在与源文件相同的目录中
- Markdown 输出：`Summary - <stem>.md`
- LaTeX 源码（可选）：`Summary - <stem>.tex`
- 最终 PDF（可选）：`Summary - <stem>.pdf`
- 任何辅助或中间生成的文件：放在同一文件夹中，命名为从 `<stem>` 派生的名称，而非 `output.*` 或 `temp.*` 等通用名称
...
- 将生成的文件散落在专用 `Summary - <stem>` 文件夹之外
...
| "把文件保存在源文件旁边更简单" | 除非用户明确选择其他位置，否则将生成的产物放入 `Summary - <stem>` 文件夹。 |
...
- 输出文件存在之前就给出最终答案
- 基于计划步骤而非已执行步骤声称完成
- 生成的文件散落在原始文件目录中而非一个 `Summary - <stem>` 文件夹
- 合并后没有验证器轮次
- 尽管概念明显需要但仍省略图表
- 以 `速查表` 为由使用不可读的压缩格式
...
| 2 | **省略页码引用** | 用户需要快速定位原文 | 每个知识点标注页码或页码范围 |
| 3 | **只写中文不写英文** | 双语术语是核心交付物 | 重要术语后附英文括号 |
| 4 | **把文件散落在源文件目录** | 文件管理混乱 | 所有文件放入 `Summary - <stem>` 文件夹 |
