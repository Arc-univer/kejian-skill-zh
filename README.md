...
输出内容聚焦**考点**而非泛泛的章节摘要：核心概念、公式、典型例题、解题技巧、易错点，每一项都附带**指向源文件对应标题的可点击链接**（全平台可用：Obsidian / VS Code / Typora）。
...
| 特性 | 说明 |
|------|------|
| **标题锚点跳转** | 每个知识点链接到源文件对应标题（`[CPU 时间公式](源文件.md#cpu-时间公式)（pp. 1468）`），点击直接定位，全平台兼容（Obsidian / VS Code / GitHub / Typora） |
| **自动分段处理** | 长文档智能按课时/主题拆分，逐段并行阅读，最后全局合并 |
| **独立验证机制** | 合并后运行全新验证器检查覆盖率、页码引用、双语术语、考试实用性；再由独立格式检查器逐行扫描链接语法、括号配对、符号完整性 |
...
### CPU 时间公式 → [CPU 时间公式及综合计算](源文件.md#cpu-时间公式及综合计算)（pp. 1468-1534）

**定义**：CPU 时间 = 指令数 × CPI × 时钟周期 → [CPU 时间公式及综合计算](源文件.md#cpu-时间公式及综合计算)（pp. 1468）

**公式**：
$$CPU\ Time = IC \times CPI \times T_c$$
*→ [CPU 时间公式及综合计算](源文件.md#cpu-时间公式及综合计算)（pp. 1470）*

**典型例题**：已知 IC=10⁶, CPI=2.5, f=2GHz，求 CPU 时间 → [CPI 的含义影响因素优化方向](源文件.md#cpi-的含义影响因素优化方向)（pp. 1500）
...
| 优先级 | 知识点 | 来源（点击跳转） |
|--------|--------|-----------------|
| 🔴 **必考** | CPU 时间公式及综合计算 | [CPU 时间公式及综合计算](源文件.md#cpu-时间公式及综合计算)（pp. 1468-1534） |
| ⚡ **重点** | CPI 的含义、影响因素、优化方向 | [CPI 的含义影响因素优化方向](源文件.md#cpi-的含义影响因素优化方向)（pp. 1476-1518） |
| 📌 **理解** | 指令执行周期（取指→译码→…→写回） | [指令执行周期](源文件.md#指令执行周期)（pp. 754-782） |
```

> **链接文字显示目标章节标题，一眼知道跳转到哪。** 括号内 `pp.` 为原课件页码。标题锚点全平台兼容（Obsidian / VS Code / GitHub / Typora）。`→` 是"参见源文件"的视觉分隔符。
...
- **段落阅读器（Section Reader）** — 每个独立课时/主题段一个，返回结构化笔记（考点、公式、**所属标题锚点**、英文术语）
- **验证器（Validator）** — 全新上下文，独立检查覆盖率、页码链接可点击性、标题锚点正确性、考试实用性。不通过则退回修改
- **格式检查器（Format Checker）** — 全新子 agent，不复用验证器；逐行扫描链接语法、括号配对、锚点格式、符号完整性。不通过则逐条修复后重检，直到零错误
...
用户：总结一下 "计算机系统概论"，我只要考点
      ↓
Skill 自动：全局阅读 → 分段 → 逐段提取考点+标题锚点 → 合并 → 内容验证 → 格式检查 → 写入文件
      ↓
...
2. 将课件 + SKILL.md 一起发给网页 AI（Claude / ChatGPT / GLM / DeepSeek）
3. 获得文字版总结（无标题跳转功能，但结构相同）
...
> **Obsidian、VS Code、GitHub 均支持标题锚点跳转**（`#heading-slug`），点击页码链接直接定位到源文件对应标题位置。
...
不会。Skill 内置强制分段机制：长文档必须先按课时/主题拆分为独立段落，逐段阅读提取，再全局合并验证。验证器会检查分段地图中的每段是否都在最终草稿中有体现，缺一段都会标记为不通过。通过后还有格式检查器逐行扫描链接语法和符号完整性，确保输出无格式错误。
...
| 页码引用可能丢失或不准 | 每个知识点带标题名链接+页码，跨平台可点击跳转 |
| 可能输出泛泛的散文摘要 | 结构化考点 + 优先级 + 易错点 |
| 英文术语可能遗漏 | 双语术语是核心交付物 |
| 一次对话即结束 | 内容验证 + 格式检查双重通过才写入文件 |
...
2. **内容与格式独立检查** — 验证器查"写得对不对"，格式检查器查"格式有没有坏"，两个关注点独立
3. **页码引用是链接，不是装饰** — 链接文字为标题原文，点击跳转到源文件对应章节，页码在括号中补充
4. **中文 + 英文双语标注** — 重要概念首次出现时必须附英文原名
5. **长文档强制分段** — 禁止一次性总结几百页的课件
6. **图表按需保留** — 只在概念确实需要图示时才包含，不堆砌
